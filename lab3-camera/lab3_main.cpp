...
void display()
{
	...
	// the view matrix is the inverse of the camera's world transform, which can be computed from the camera base vectors and position
	...
	mat4 r = glm::rotate(float(currentTime * M_PI*-0.7), vec3(0.0f, 1.0f, 0.0f)); //*-0.7 to control velocirt and direction (sign)
	mat4 t = glm::translate(vec3(20.0f, 0.0f, 0.0f));
	mat4 t2 = glm::translate(vec3(r * vec4(10.f, 0, 0, 1.f))); // translate to the position on the circle with radius 10, then rotate the car to face the direction of movement
	mat4 carModelMatrix2 = t * t2 * r;
